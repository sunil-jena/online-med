                    ...
                    >
                        ...
                        <motion.div layoutId="emailHeader" className="relative mt-2">
                            <input
                                ...
                                className={`input pr-11 ${emailForm.touched.email && emailForm.errors.email ? "input-error" : ""} ${emailForm.isSubmitting ? "placeholder-brand" : ""}`}
                        ...
                        {emailForm.touched.email && emailForm.errors.email && (
                            <div role="alert" className="mt-3 text-[#dc2626] shake">{emailForm.errors.email}</div>
                        )}

                        <BackNext onBack={() => router.push('/')} onNext={() => emailForm.submitForm()} nextDisabled={!emailForm.isValid || emailForm.isSubmitting} />
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <AnimatePresence initial={false}>
                {step === "otp" && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0, scale: 1, transition: { duration: 0.26, ease: [0.22, 0.61, 0.36, 1] } }}
                        exit={{ opacity: 0, y: -8, transition: { duration: 0.2 } }}
                    >
                        <motion.div layoutId="emailHeader" className="flex items-center justify-between px-4 py-3 bg-(--color-brand-tint)">
                            <div className="flex items-center gap-2">
                                <div className="label">Email</div>
                                <span key={sheenKey} className="font-medium brand-sheen-rtl">{email}</span>
                            </div>
                            <button type="button" className="dotted-link text-sm" onClick={backToEmail}>Change</button>
                        </motion.div>
                        ...
                        <div className="border-t border-(--color-line) px-4 py-5">
                            ...
                            <div className="mt-4 text-sm text-(--color-ink-muted)">
                                {cooldown === 0 ? (
                                    <>Didn’t receive a code? <button className="link" onClick={resend}>Send again</button></>
                                ) : (
                            ...
                            <BackNext onBack={backToEmail} onNext={() => otpForm.submitForm()} nextDisabled={!otpForm.isValid || otpForm.isSubmitting} />
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
